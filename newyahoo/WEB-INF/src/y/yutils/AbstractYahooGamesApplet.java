...
import y.ydialogs.AppletLoginDialog;
...
public abstract class AbstractYahooGamesApplet extends AbstractYahooApplet
	...
	protected boolean hasAppletCredentials() {
		return cookie != null && cookie.length() > 0 && ycookie != null
				&& ycookie.length() > 0;
	}

	protected String extractIdFromCookie() {
		if (cookie == null)
			return "";
	...
	protected void promptForAppletLogin(String message) {
	...
	public void submitAppletLogin(String username, String password) {
	...
	public void initProperties() {
		...
		if (hasAppletCredentials()) {
			...
			promptForAppletLogin(loginErrorMessage != null ? loginErrorMessage : "Please sign in");
		}
	...
	public void processMessages(DataInputStream datainputstream, int state) {
		...
		try {
			...
			disconnected = true;
			loginErrorMessage = _lcls22.getMessage();
			cookie = "";
			ycookie = "";
			if (loginDialog != null)
				loginDialog.setBusy(false);
			promptForAppletLogin(_lcls22.getMessage());
		}
