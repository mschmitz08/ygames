...
public abstract class YahooServer extends Thread {
	...
	public YahooServer(YahooRoomHandler handler, int port,
		...
		closing = false;
		DebugLog.log("YahooServer ctor name=" + getName() + " port=" + port + " roomCount=" + rooms.size());
		start();
	}
	...
	public YahooRoom getRoom(String yport) {
		YahooRoom room = rooms.get(yport);
		DebugLog.log("YahooServer.getRoom yport=" + yport + " -> " + (room == null ? "null" : room.getClass().getName() + " index=" + room.getIndex()));
		return room;
	}
	...
	@Override
	public void run() {
		DebugLog.log("YahooServer.run ENTER name=" + getName() + " port=" + port);
		try {
			server = new ServerSocket(port);
			DebugLog.log("YahooServer listening port=" + port);
			do {
				Socket socket = server.accept();
				DebugLog.log("YahooServer accepted remote=" + socket.getRemoteSocketAddress() + " local=" + socket.getLocalSocketAddress());
				if (closing) {
					DebugLog.log("YahooServer closing=true after accept; closing socket");
					socket.close();
					socket = null;
					return;
				}
				YahooServerConnection connection = createConnection(socket);
				DebugLog.log("YahooServer created connection class=" + (connection == null ? "null" : connection.getClass().getName()));
				connections.add(connection);
				DebugLog.log("YahooServer active connections=" + connections.size());
			}
			while (true);
		}
		catch (SocketException e) {
			DebugLog.log("YahooServer.run SocketException", e);
		}
		catch (EOFException e) {
			DebugLog.log("YahooServer.run EOFException", e);
		}
		catch (Throwable e) {
			DebugLog.log("YahooServer.run Throwable", e);
			e.printStackTrace();
		}
		finally {
			DebugLog.log("YahooServer.run FINALLY close()");
			close();
		}
	}
