...
public abstract class YahooRoom {
	...
	public YahooRoom(YahooRoomHandler handler, int index, String yport) {
		...
		DebugLog.log("YahooRoom ctor class=" + getClass().getName() + " index=" + index + " yport=" + yport + " tables=" + tables.length);
	}
	...
	public void addId(final YahooConnectionId id) {
		...
		try {
			ids.add(id);
			idTable.put(name, id);
			DebugLog.log("YahooRoom.addId stored id name=" + name + " roomCount=" + ids.size());
		}
		finally {
			ids.writeUnlock();
		}
		...
		// blue message
		DebugLog.log("YahooRoom.addId sending welcome/room state to name=" + name + " tablesFreeCheck done");
		...
		DebugLog.log("YahooRoom.addId EXIT room=" + yport + " name=" + name);
	}
	...
	public boolean doInvite(YahooConnectionId id, String name, int table) {
		...
		YahooConnectionId id1 = idTable.get(name);
		if (id1 == null) {
			alert(id, name + " n�o foi encontrado no servidor");
			return false;
		}
		if (id1.isDeclineAllInviations()) {
			alert(id, name + " n�o est� aceitando convites");
			return false;
		}
		if (id1.isIgnored(idName))
			return false;
	...
	public boolean doJoinTable(YahooConnectionId id, int table) {
		DebugLog.log("YahooRoom.doJoinTable room=" + yport + " name=" + (id == null ? "null" : id.getName()) + " table=" + table);
		if (table < 1 || table > 255)
			return false;

		boolean result = tables[table].joinId(id);
		DebugLog.log("YahooRoom.doJoinTable result=" + result + " table=" + table);
		return result;
	}
	...
	public void doPlayNow(YahooConnectionId id) {
		DebugLog.log("YahooRoom.doPlayNow room=" + yport + " name=" + (id == null ? "null" : id.getName()));
		for (int i = 1; i < tables.length; i++) {
			if (!doJoinTable(id, i))
				continue;
			return;
		...
		DebugLog.log("YahooRoom.doPlayNow no existing table joined; attempted doMakeTable");
	}
	...
	public void parseData(YahooConnectionId id, byte byte0,
		...
		DebugLog.log("YahooRoom.parseData room=" + yport + " name=" + (id == null ? "null" : id.getName()) + " cmd=" + byte0 + " char=" + (char)byte0 + " state=" + (id == null ? "null" : Integer.toString(id.getState())));
		if (byte0 != 'H')
